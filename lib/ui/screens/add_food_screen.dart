...
class AddFoodScreen extends StatefulWidget {
  ...
  final bool isLoggingOnly;

  const AddFoodScreen({
    super.key,
    this.existingItem,
    this.isLoggingOnly = false,
  });
...
class _AddFoodScreenState extends State<AddFoodScreen> {
  ...
  void initState() {
    ...
    _totalServingsController = TextEditingController(
      text: item != null ? "1" : "",
    );
  ...
  Future<void> _save() async {
    if (_formKey.currentState!.validate()) {
      ...
      final pricePerServing =
          totalPrice / (totalServings > 0 ? totalServings : 1);
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: _numField(_servingSizeController, "Serving Size"),
                  ),
              ...
              Row(
                children: [
                  Expanded(
                    child: _numField(_priceController, "Total Price (\$)"),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: _numField(
                      _totalServingsController,
                      "Total Servings",
                    ),
                  ),
                ],
              ...
              ElevatedButton(
                onPressed: _save,
                child: Text(
                  widget.isLoggingOnly ? "Log to Diary" : "Save to Pantry",
                ),
              ),
            ],
          ),
